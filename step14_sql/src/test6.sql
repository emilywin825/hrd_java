...
- 구조만복사 - 
create table 테이블명1 
as  select  *  from 테이블명2  where 1=0 --> where 참이 아닌 조건을 삽입
---------------------------------------------------------------------------------
[시퀀스]
...
--ex2) 테이블명 : user1
create table user1( --칼럼명, 데이터, 제약조건
idx     number  primary key, -- 중복 x, null x
id      varchar2(10) unique,-- 중복 x
...
marriage   char(1)  default 'N'  check(marriage in('Y','N'))); --default 제약조건, check 제약 조건 둘 다 사용 중
...
select constraint_name, constraint_type
from user_constraints
where table_name='USER2'; --이름 대문자로
...
values (1,'apple','kim','010-000-0000','서울',75,100,'2010-08-01','Y');

insert into user1(idx, id, name, phone, address, score, subject_code, hire_date, marriage)
values (2,'bbb','kim','010-000-0000','서울',75,100,'2010-08-01','Y');

insert into user1(idx, id, name, phone, address, score, subject_code, hire_date, marriage)
values (1,'apple','kim','010-000-0000','서울',75,100,'2010-08-01','Y'); --유니크제약조건 위배 (=무결성제약조건 위배)

insert into user1(idx, id, name, phone, address, score, subject_code, hire_date, marriage)
values (2,'bbb','010-000-0000','서울',75,100,'2010-08-01','Y'); --데이터의 개수가 컬럼의 개수와 일치하지 않음

insert into user1(idx, id, name, phone, address, score, subject_code, hire_date, marriage)
values (2,'bbb','lee','010-000-0000','서울',125,100,'2010-08-01','Y'); --heck constraint (EDU.SYS_C007012) violated
                                                                      --체크제약조건 위배, 값의 범위 위배
                                                                      
insert into user1(idx, id, name, phone, address, score, subject_code, hire_date, marriage)
values (2,'bbb','010-000-0000','서울',75,100,'2010-08-01','A'); --not enough values --marriage 값 범위 위배
...
create table exam2(
   ...
   id      varchar2(10),
   name    varchar2(10)    
   constraint NONAME2 not null,
   constraint PNUM2 primary key(num),
   constraint UNID2 unique(id)
); --제약조건 이름 중복 불가

--제약 조건 삭제
alter table exam2 drop constraints noname2 cascade; --cascade : 수정하고자 하는 테이블을 참조하고 있는 테이블이 있다면 그 테이블도 수정
...
drop table user3 ;
drop table user3 purge; -- 휴지통에 넣지 않고 삭제

flashback table user3 to before drop; -- 휴지통에서 되살리기

select * from tab --테이블 종류 확인
...
from user_constraints
where table_name='USER4' -->체크제약 조건이 복사된는 것이 아니라 not null제약조건이 복사되는 것임
...
select idx_num.nextval from dual; --idx_num.nextval : 동작 실행 시킴
...
references dept on delete set null --테이블 레벨 제약조건
